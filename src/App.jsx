...
const CSS=`
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  ::-webkit-scrollbar{width:5px;height:5px;}::-webkit-scrollbar-thumb{background:${G.accentDark};border-radius:3px;}
      ...
      <div style={{background:G.card,borderRadius:16,width:"100%",maxWidth:620,maxHeight:"92vh",overflowY:"auto",boxShadow:"0 16px 48px rgba(0,0,0,.22)"}}>
        ...
        <div style={{padding:20,display:"flex",flexDirection:"column",gap:12}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:4}}>
            ...
            <select value={prosProcs.find(p=>p.name===f.proc)?f.proc:"__custom__"} onChange={e=>{if(e.target.value==="__custom__")setF(p=>({...p,proc:""}));else setF(p=>({...p,proc:e.target.value}));}} style={{border:`1.5px solid ${G.border}`,borderRadius:8,padding:"8px 11px",fontSize:14,outline:"none",color:G.text,background:"#fff"}}>
              <option value="">Selecione o procedimento...</option>
...
export default function App(){
  ...
  useEffect(()=>{
    supabase.load().then(data=>{
      if(data){
        try{
          isSaving.current=true; // bloqueia save durante o load
      ...
      // Libera save apenas 1 segundo após load completar
      setTimeout(()=>{
